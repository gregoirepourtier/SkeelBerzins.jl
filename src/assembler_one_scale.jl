...
"""
    assemble_one_scale(du, u, problem, t)
...
"""
function assemble_one_scale!(du, u, pb::ProblemDefinition{m, npde, singular}, t) where {m, npde, singular}
    ...
    for i ∈ 2:(pb.Nx - 1)
        ...
        for j ∈ 1:npde
            idx_u = j + (i - 1) * npde
            @views assemble_local!(du, u, i, idx_u, j, pb, cl[j], fl[j], sl[j], cr[j], fr[j], sr[j], pl[j],
    ...
    for i ∈ 1:npde
        ...
        @views assemble_right_bd!(du, u, pb.Nx, idx_u, pb, cl[i], fl[i], sl[i], pr[i], qr[i],
...
"""
function mass_matrix_one_scale(pb::ProblemDefinition{m, npde, singular}) where {m, npde, singular}
