...
function two_scale_assembler!(du, u, pb, t, idx_u, idx_uP1, idx_micro)
    ...
    pl, ql, pr, qr = bdfun(mesh[1], u[idx_u + 1], mesh[end], u[idx_uP1 - 1], t)
    @views pr = coupling(pb.xmesh[idx_micro], t, u[idx_u], u[idx_u+1 : idx_uP1-1])

    interpolant, d_interpolant = interpolation(mesh[1], u[idx_u + 1], mesh[2], u[idx_u + 2], ξ[1], singular, m)
    ...
    if singular # ignores the given boundary condition to enforce the symmetry condition
        for i ∈ 1:npde
            if cl[i] ≠ 0
                du[idx_u + i] = ((m+1)*fl[i]/ξ[1] + sl[i]) / cl[i]
            else # stationary equation: set the corresponding coefficient in the mass matrix to 0 to generate a DAE
                du[idx_u + i] = ((m+1)*fl[i]/ξ[1] + sl[i])
            end
        end
    else # Regular Case
        for i ∈ 1:npde
            if ql[i] ≠ 0 && cl[i] ≠ 0
                du[idx_u + i] = (pl[i] + ql[i]/mesh[1]^(m) * ((ξ[1]^m)*fl[i] + frac*sl[i])) / (ql[i]/(mesh[1]^(m))*frac*cl[i])
            elseif ql[idx_u + i] ≠ 0 && cl[i] == 0 # stationary equation: set the corresponding coefficient in the mass matrix to 0 to generate a DAE
                du[idx_u + i] = (pl[i] + ql[i]/mesh[1]^(m) * ((ξ[1]^m)*fl[i] + frac*sl[i]))
